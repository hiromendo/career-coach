...
# (Using the section_content_map from the previous step - ensure this is defined correctly)
section_content_map = [
    # Section 1
...
# --- Apply Custom CSS for visual cues ---
st.markdown("""
<style>
    /* Style for the locked placeholder */
    div.locked-section {
        margin-left: 5px; /* Match expander indentation */
        padding: 10px 10px 10px 13px; /* Match expander padding roughly */
        border-left: 3px solid #e0e0e0; /* Match expander left border */
        border-radius: 5px; /* Match expander radius */
        background-color: #CFCFCF; /* Slightly off-white background */
        opacity: 0.7; /* Make it look disabled */
        margin-bottom: 10px; /* Spacing */
    }
    div.locked-section h4 {
        margin-bottom: 0.2rem; /* Reduce space below header */
        color: #555; /* Grey out text */
    }
    div.locked-section small {
         color: #777; /* Grey out caption */
    }
    /* Style for unlocked expanders */
    div[data-testid="stExpander"] {
        border-left: 3px solid #ccc; /* Slightly darker border when active */
        margin-left: 5px;
        padding-left: 10px;
        margin-bottom: 10px;
    }
    /* Reduce excessive space below headers */
     h1, h2, h3 {
        margin-bottom: 0.5rem !important; /* Use !important cautiously */
    }
</style>
""", unsafe_allow_html=True)


# --- Loop Through Content Map to Display Expanders or Locked Placeholders ---
for i, (key, content, link_info) in enumerate(section_content_map):
    ...
    if key in section_headers and section_headers[key] != current_header:
        ...
        # Add extra space before new header if it's not the very first one
        if i > 0:
            st.markdown("<br>", unsafe_allow_html=True)
        st.header(current_header)
        # st.divider() # Removed divider below header for cleaner look

    # Determine if this section should be enabled/unlocked
    is_unlocked = (i == 0) or st.session_state[expander_keys_ordered[i-1]]

    if is_unlocked:
        # Add a checkmark prefix if the section is already completed
        expander_label = f"✅ {key}" if st.session_state[key] else key

        # Render the interactive expander
        with st.expander(expander_label):
            st.markdown(content) # Display content

            if link_info: # Display page link if provided
                page_file, label, icon = link_info
                st.page_link(page_file, label=label, icon=icon)
                st.markdown("---")

            # Checkbox is always enabled when the expander is unlocked
            st.checkbox("Mark as reviewed",
                         key=key,
                         help="Mark section as reviewed to unlock the next.")
    else:
        # Render a locked placeholder label using a styled div
        st.markdown(f"""
...
st.divider() # Add divider after the last section loop completes
