...
from streamlit_d3graph import d3graph # Import the library
...
# Group keys by section for easier graph generation
expander_keys_by_section = {
    "sec1": [
...
# Flatten the dictionary values to get the original list for progress tracking
expander_keys = [key for section_keys in expander_keys_by_section.values() for key in section_keys]
...
# --- NEW: Section for Visual Roadmap Graph ---
st.header("Visual Roadmap Overview")
st.markdown("This graph shows the main sections and topics covered in this guide. You can drag nodes around.")

# Define Nodes for the graph
nodes = []
# Root Node
nodes.append({'id': 'root', 'label': 'Re-Entering Accounting (Portland)', 'size': 400, 'color': '#FF6347'}) # Tomato color for root

# Section Nodes
section_labels = {
    "sec1": "1. Step-by-Step Plan",
    "sec2": "2. Licensing",
    "sec3": "3. Education & Prep",
    "sec4": "4. Job Search",
    "sec5": "5. Networking & Orgs",
    "sec6": "6. Events & Fairs"
}
section_colors = ['#4682B4', '#6A5ACD', '#3CB371', '#FFA500', '#D2691E', '#8A2BE2'] # SteelBlue, SlateBlue, MediumSeaGreen, Orange, Chocolate, BlueViolet

for i, (sec_id, label) in enumerate(section_labels.items()):
    nodes.append({'id': sec_id, 'label': label, 'size': 250, 'color': section_colors[i]})

# Topic Nodes (using expander keys as IDs and labels)
topic_color = '#ADD8E6' # LightBlue for topics
for sec_id, keys in expander_keys_by_section.items():
    for key in keys:
        # Use the full key string as both ID and label for simplicity
        nodes.append({'id': key, 'label': key, 'size': 100, 'color': topic_color})

# Define Links for the graph
links = []
# Links from Root to Sections
for sec_id in section_labels.keys():
    links.append({'source': 'root', 'target': sec_id})

# Links from Sections to Topics
for sec_id, keys in expander_keys_by_section.items():
    for key in keys:
        links.append({'source': sec_id, 'target': key})

# Configure the graph appearance
config = {
    'width': 1000,  # Adjust width as needed
    'height': 700, # Adjust height as needed
    'directed': True, # Show direction from root -> section -> topic
    'nodeHighlightBehavior': True,
    'linkHighlightBehavior': True,
    'highlightColor': "blue",
    'collapsible': True, # Allows collapsing/expanding nodes
    'node': {'labelProperty': 'label'}, # Ensure labels are shown
    'link': {'labelProperty': 'label', 'renderLabel': False}, # Don't render link labels if any
    'd3': { # Fine-tune D3 force simulation
        'gravity': -100,
        'linkStrength': 0.8,
        'linkDistance': 100, # Increase distance between nodes
        'charge': -200    # Increase repulsion between nodes
        }
    }

# Display the graph
try:
    d3graph(nodes=nodes, links=links, config=config)
except Exception as e:
    st.error(f"Could not generate graph. Error: {e}")
    st.warning("Ensure the 'streamlit-d3graph' library is installed (`pip install streamlit-d3graph`).")

st.divider()
# --- END of Visual Roadmap Graph Section ---
